...
## Testing the backend
1. Create/activate the virtualenv and install dev dependencies:
   ```bash
   python3 -m venv .venv && source .venv/bin/activate
   pip install -r requirements-dev.txt
   ```
2. Run the FastAPI smoke tests:
   ```bash
   pytest
   ```
   The suite exercises `/health`, scan job creation, printer test-page submission, and history listing using the current in-memory stubs.
